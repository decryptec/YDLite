...
def longer_than(info, min_duration, *, incomplete):
    """Download only videos longer than min_duration seconds"""
...
def format_selector(ctx, preferred_ext="mp4"):
    """Select best video+audio combo avoiding mkv"""
...
def index():
    ...
    if request.method == "POST":
        ...
        if mode == "info":
            ...
            result = "Some videos failed" if error_code else f"Audio extracted as {codec}"
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                error_code = ydl.download([url])
            result = "Some videos failed" if error_code else f"Filtered download (>{min_duration}s) complete"

        elif mode == "logger":
            debug = request.form.get("debug") == "true"
            ydl_opts = {'logger': MyLogger(debug=debug), 'progress_hooks': [my_hook]}
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                error_code = ydl.download([url])
            result = "Some videos failed" if error_code else "Download with logger complete"
            ...
            with yt_dlp.YoutubeDL() as ydl:
                ...
                error_code = ydl.download([url])
            result = "Some videos failed" if error_code else f"Custom PostProcessor ran ({when})"

        elif mode == "format_selector":
            preferred_ext = request.form.get("preferred_ext", "mp4")
            ydl_opts = {'format': lambda ctx: format_selector(ctx, preferred_ext)}
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                error_code = ydl.download([url])
            result = "Some videos failed" if error_code else f"Custom format selector download ({preferred_ext}) complete"
